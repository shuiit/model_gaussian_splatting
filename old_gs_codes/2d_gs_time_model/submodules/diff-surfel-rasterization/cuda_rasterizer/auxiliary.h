...
__device__ const float near_n = 0.09;
__device__ const float far_n = 0.4;
__device__ const float FilterSize = 0.707106; //0.1;//2;// sqrt(2) / 2
__device__ const float FilterInvSquare = 2.0f;//100;//0.25;//
	...
	float3& p_view)
{
	...
	if (p_view.z <= 0.05f)// || ((p_proj.x < -1.3 || p_proj.x > 1.3 || p_proj.y < -1.3 || p_proj.y > 1.3)))
