...
class GaussianModel:
    ...
    #     return self.body_location
    
    # @property
    # def get_right_wing_angles(self):
    #     return self.right_wing_angles
    
    # @property
    # def get_left_wing_angles(self):
    #     return self.left_wing_angles
    ...
    def update_model_location(self):
        ...
        return self.get_xyz + self.body_angles
        # self.body_location = self.body_location*0.2

        # self.skin_model.bones[0].joint.set_local_rotation(self.body_angles)
        # self.skin_model.bones[0].update_rotation()
        # points_homo = torch.cat([self.get_xyz, torch.ones(self.get_xyz.shape[0], 1).cuda()], dim=1)
        # rotated_points = [joint.rotate_to_new_position(weight[:, None], points_homo) for weight, joint in zip(self.skin_model.weights.T, self.skin_model.bones)]
        # return torch.sum(torch.stack(rotated_points), dim=0)[:, 0:3]
        # joint.set_local_rotation(self.right_wing_angles)
        # joint.set_local_rotation(self.left_wing_angles)


        # rotation_to_update = [self.body_angles,self.right_wing_angles,self.left_wing_angles]
        # joints_to_update = [self.skin_model.bones[0],self.skin_model.bones[3],self.skin_model.bones[4]]
        # [joint.set_local_rotation([0,-rotation[1],0]) for joint,rotation in zip(joints_to_update,rotation_to_update)]
        # joints_to_update[0].set_local_translation(self.body_location) 
        # [joint.update_rotation() for joint in self.skin_model.list_of_joints]
        
        # points_homo = torch.cat([self.get_xyz, torch.ones(self.get_xyz.shape[0], 1).cuda()], dim=1)
        # rotated_points = [joint.rotate_to_new_position(weight[:, None], points_homo) for weight, joint in zip(self.skin_model.weights.T, self.skin_model.bones)]
        # return torch.sum(torch.stack(rotated_points), dim=0)[:, 0:3]

        # return torch.tensor(self.skin_model.rotate_skin_points()).float().cuda()
